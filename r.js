...
const syntax = {
  ...
  placeholder: e => e.replace('"', '').replace('placeholder=', ''),
}
...
const extract_text = syntax.html
...
const res = names.filter(filter_unique).map(toTranslate)

const create_json = el => fs.writeFile(config.result_trad_file, JSON.stringify(el, null, '\t'), _ => _);
